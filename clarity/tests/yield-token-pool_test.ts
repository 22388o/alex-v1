...
const wrongExpiry = 70000 * ONE_16
const anotherExpiry = 80875 * ONE_16
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let result = wbtcToken.mintFixed(deployer, deployer.address, 100000 * ONE_16);
        result.expectOk();
        result = wbtcToken.mintFixed(deployer, wallet_1.address, 100000 * ONE_16);
        result.expectOk(); 
        result = yieldWBTC.mintFixed(deployer, expiry, 10000 * ONE_16, deployer.address);
        ...
        position['balance-token'].expectUint(1000*ONE_16);
        position['balance-yield-token'].expectUint(0);
        position['balance-virtual'].expectUint(1000*ONE_16);

        let listed = 100000000;

        //Add extra liquidity
        result = YTPTest.addToPosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 10*ONE_16, Number.MAX_SAFE_INTEGER);
        position = result.expectOk().expectTuple();
        position['supply'].expectUint(10*ONE_16);
        position['balance-token'].expectUint(10*ONE_16);
        position['balance-yield-token'].expectUint(0);
        position['balance-virtual'].expectUint(10*ONE_16);

        // Check pool details and print
        call = await YTPTest.getPoolDetails(expiry, yieldwbtcAddress);
        position = call.result.expectOk().expectTuple();
        position['total-supply'].expectUint(1010*ONE_16);
        position['balance-token'].expectUint(1010*ONE_16);
        position['balance-yield-token'].expectUint(0);
        position['balance-virtual'].expectUint(1010*ONE_16);        
        
        // Remove all liquidlity
        result = YTPTest.reducePosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, ONE_16);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(1010*ONE_16);
        ...
        result = YTPTest.addToPosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 1000*ONE_16, Number.MAX_SAFE_INTEGER);
        position = result.expectOk().expectTuple();
        position['supply'].expectUint(1000*ONE_16);
        position['balance-token'].expectUint(1000*ONE_16);
        position['balance-yield-token'].expectUint(0);
        position['balance-virtual'].expectUint(1000*ONE_16);     
        ...
        call = await YTPTest.getYgivenX(expiry, yieldwbtcAddress, 1*ONE_16);
        ...
        call = chain.callReadOnlyFn("yield-token-pool", "get-yield", 
            ['u' + BigInt(expiry), types.principal(yieldwbtcAddress)
            ], deployer.address);
        call.result.expectOk().expectUint(5)

        // zero rate environment, so yield-token and token are at parity.
        call = await YTPTest.getXgivenY(expiry, yieldwbtcAddress, 2*ONE_16);
        call.result.expectOk().expectUint(199971366)

        // sell some yield-token
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 2*ONE_16, 0);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(199971366);
        position['dy'].expectUint(2*ONE_16);
        ...
        position['balance-yield-token'].expectUint(2*ONE_16);
        position['balance-virtual'].expectUint(1000*ONE_16);         
        ...
        result = YTPTest.swapXForY(deployer, expiry, yieldwbtcAddress, wbtcAddress, ONE_16, 0);
        position = result.expectOk().expectTuple()
        position['dx'].expectUint(ONE_16);
        position['dy'].expectUint(100028858);        

        // attempt to sell more than max allowed yield token (50% of pool) must throw an error
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 501*ONE_16, 0);
        ...
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 29*ONE_16, 0);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(2900528265);
        position['dy'].expectUint(29*ONE_16);                      
        ...
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 100*ONE_16, 0);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(10001386469);
        position['dy'].expectUint(100*ONE_16);       
        ...
        //buy back some yield token
        result = YTPTest.swapXForY(deployer, expiry, yieldwbtcAddress, wbtcAddress, 100*ONE_16, 0);
        position =result.expectOk().expectTuple();
        ...
        call = chain.callReadOnlyFn(ytpyieldwbtcAddress, "get-balance-fixed", 
            ['u' + BigInt(expiry), types.principal(deployer.address)
            ], deployer.address);
        call.result.expectOk().expectUint(100000000000);   

        // Remove some liquidlity
        result = YTPTest.reducePosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 0.5*ONE_16);
        position = result.expectOk().expectTuple();
        ...
        result = YTPTest.swapXForY(deployer, expiry, yieldwbtcAddress, wbtcAddress, 19*ONE_16, 0);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(19*ONE_16);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result = YTPTest.createPool(wallet_1, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, multisigytpyieldwbtc, 1000*ONE_16, 1000*ONE_16);
        ...
        result = YTPTest.addToPosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, wrongPooltokenAddress, 10*ONE_16, Number.MAX_SAFE_INTEGER);
        result.expectErr().expectUint(2026);

        // non-deployer can add liquidity
        result = YTPTest.addToPosition(wallet_1, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 10*ONE_16, Number.MAX_SAFE_INTEGER);
        result.expectOk();
        
        //if wrong pool token is supplied, throw an error
        result = YTPTest.reducePosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, wrongPooltokenAddress, ONE_16);
        result.expectErr().expectUint(2026);        
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        call = await YTPTest.getXgivenYield(expiry, yieldwbtcAddress, 0.001*ONE_16);
        call.result.expectErr().expectUint(2002);
        call = await YTPTest.getYgivenYield(expiry, yieldwbtcAddress, 0.001*ONE_16);          
        ...
        call = await YTPTest.getYgivenYield(expiry, yieldwbtcAddress, 0.0005*ONE_16);                  
        call.result.expectErr().expectUint(2002);
        call = await YTPTest.getXgivenYield(expiry, yieldwbtcAddress, 0.0005*ONE_16);        
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result = wbtcToken.mintFixed(deployer, wallet_1.address, 100000 * ONE_16);
        result.expectOk(); 
        result = wbtcToken.mintFixed(deployer, wallet_2.address, 10 * ONE_16);
        result.expectOk(); 
        result = yieldWBTC.mintFixed(deployer, expiry, 10000 * ONE_16, deployer.address);
        result.expectOk().expectBool(true);
        result = usdaToken.mintFixed(deployer, deployer.address, 100000000 * ONE_16);
        result.expectOk();
        result = usdaToken.mintFixed(deployer, wallet_1.address, 200000 * ONE_16);
        result.expectOk();   
        result = usdaToken.mintFixed(deployer, wallet_2.address, 1000 * ONE_16);
        result.expectOk();      
        ...
        let call = await YTPTest.getPoolDetails(expiry, yieldwbtcAddress);
        let position:any = call.result.expectOk().expectTuple();
        position['total-supply'].expectUint(1000*ONE_16);
        position['balance-token'].expectUint(1000*ONE_16);
        position['balance-yield-token'].expectUint(0);
        position['balance-virtual'].expectUint(1000*ONE_16);

        result = YTPTest.addToPosition(wallet_2, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 10*ONE_16, Number.MAX_SAFE_INTEGER);
        position = result.expectOk().expectTuple();
        position['supply'].expectUint(10*ONE_16);
        position['balance-token'].expectUint(10*ONE_16);
        position['balance-yield-token'].expectUint(0);
        ...
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, ONE_16, 0);
        position =result.expectOk().expectTuple();
        ...
        position['balance-yield-token'].expectUint(ONE_16);
        position['balance-virtual'].expectUint(1010*ONE_16);

        call = await ytpPoolToken.balanceOf(expiry, deployer.address);
        call.result.expectOk().expectUint(1000*ONE_16);    // u100000000000

        call = await ytpPoolToken.balanceOf(expiry, wallet_2.address);
        call.result.expectOk().expectUint(10*ONE_16);
        ...
        ROresult.result.expectOk().expectUint(1000*ONE_16);
        ...
        position['fee-rate-yield-token'].expectUint(0.1*ONE_16);
        position['fee-rate-token'].expectUint(0.1*ONE_16);
        position['fee-rebate'].expectUint(0.5*ONE_16);
        
        call = await YTPTest.getYield(expiry, yieldwbtcAddress);
        call.result.expectOk().expectUint(1355);

        // fee-yield = yield * fee-rate-token = 1355 * 0.1*ONE_16 = 135 (round-down) // (contract-call? .math-fixed-point mul-down 1335 u10000000)
        // lambda = ONE_16 - fee-yield = ONE_16 - 135 = 9999865 (non-floating point)
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result = YTPTest.addToPosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 1000000*ONE_16, Number.MAX_SAFE_INTEGER);
        ...
        result = YTPTest.reducePosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 101*ONE_16);
        position =result.expectErr().expectUint(5000)

        // Another user attempts to reduce liquidity with not enough pool token 
        result = YTPTest.reducePosition(wallet_2, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 1*ONE_16);
        position =result.expectErr().expectUint(1)
        ...
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 0.0000001 * ONE_16, 0);
        position =result.expectErr().expectUint(2020);

        // Fixed
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 0.001 * ONE_16, 0);
        position =result.expectOk().expectTuple();
        ...
        // Swap
        result = YTPTest.swapXForY(deployer, expiry, yieldwbtcAddress, wbtcAddress, 0.1 * ONE_16, 0);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(0.1 * ONE_16);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result = YTPTest.swapYForX(deployer, expiry, yieldwbtcAddress, wbtcAddress, 10 * ONE_16, 0);
        position =result.expectOk().expectTuple();
        position['dx'].expectUint(1000041873);
        position['dy'].expectUint(10 * ONE_16);

        // Check pool details and print
        call = await YTPTest.getPoolDetails(expiry, yieldwbtcAddress);
        position = call.result.expectOk().expectTuple();
        position['total-supply'].expectUint(1000*ONE_16);
        position['balance-token'].expectUint(1000*ONE_16 - 1000041873);
        position['balance-yield-token'].expectUint(10 * ONE_16);
        position['balance-virtual'].expectUint(1000*ONE_16);  

        // make sure wallet_1 does not have any yield-token
        call = chain.callReadOnlyFn(yieldwbtcAddress, "get-balance", 
            ['u' + BigInt(expiry), types.principal(wallet_1.address)
            ], wallet_1.address);
        call.result.expectOk().expectUint(0);            
        
        //Add extra liquidity with secondary buying of yield-token
        result = YTPTest.buyAndAddToPosition(wallet_1, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 10*ONE_16, Number.MAX_SAFE_INTEGER);
        position = result.expectOk().expectTuple();
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result = yieldWBTC.mintFixed(deployer, expiry, 10000 * ONE_16, deployer.address);
        result.expectOk().expectBool(true);      
        result = yieldWBTC.mintFixed(deployer, anotherExpiry, 10000 * ONE_16, deployer.address);
        result.expectOk().expectBool(true);     
        ...
        position = call.result.expectOk().expectTuple();
        position['total-supply'].expectUint(1000*ONE_16);
        position['balance-token'].expectUint(1000*ONE_16 - 1000037034);
        position['balance-yield-token'].expectUint(10 * ONE_16);
        position['balance-virtual'].expectUint(1000*ONE_16);  
        ...
        // create another ytp
        result = YTPTest.createPool(deployer, anotherExpiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, multisigytpyieldwbtc, 1000*ONE_16, 1000*ONE_16);
        result.expectOk().expectBool(true);       
        // inject some yield-token to pool
        result = YTPTest.swapYForX(deployer, anotherExpiry, yieldwbtcAddress, wbtcAddress, 10 * ONE_16, 0);
        position =result.expectOk().expectTuple();
        ...
        position = call.result.expectOk().expectTuple();
        position['total-supply'].expectUint(1000*ONE_16);
        position['balance-token'].expectUint(1000*ONE_16 - 1000063164);
        position['balance-yield-token'].expectUint(10 * ONE_16);
        position['balance-virtual'].expectUint(1000*ONE_16);          
        
        //Add extra liquidity with secondary buying of yield-token
        result = YTPTest.rollPosition(deployer, expiry, yieldwbtcAddress, wbtcAddress, ytpyieldwbtcAddress, 0.5*ONE_16, anotherExpiry);
        position = result.expectOk().expectTuple();
        ...
        call = await YTPTest.getPoolDetails(anotherExpiry, yieldwbtcAddress);
        position = call.result.expectOk().expectTuple();
        position['total-supply'].expectUint(1000*ONE_16 + 50354657000);
        // a bit more than 148900862764 = 1000*ONE_16 - 1000067035 + 49900926876, due to buy-and-add-to-position
        position['balance-token'].expectUint(148999867325); 
        // a bit less then 1453675742 = 0 + 10 * ONE_16 + 453674699, due to buy-and-add-to-position
        position['balance-yield-token'].expectUint(1354633808);
        position['balance-virtual'].expectUint(1000*ONE_16 + 50354657156);          
    }
});