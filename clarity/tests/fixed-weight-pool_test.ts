...
const weightX = 0.5e16;
const weightY = 0.5e16;
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result = FWPTest.reducePosition(deployer, wstxAddress, wbtcAddress, weightX, weightY, fwpwstxwbtcAddress, ONE_16);
        position = result.expectOk().expectTuple();
        ...
        result = FWPTest.swapXForY(deployer, wbtcAddress, usdaAddress, weightX, weightY, 91*ONE_16, 0);
        position = result.expectErr().expectUint(4002);

        // swap some wbtc into usda
        result = FWPTest.swapXForY(deployer, wbtcAddress, usdaAddress, weightX, weightY, ONE_16, 0);
        position = result.expectOk().expectTuple();
        position['dx'].expectUint(ONE_16);
        position['dy'].expectUint(4999991000000);    
        
        // swap some usda into wbtc
        result = FWPTest.swapYForX(deployer, wbtcAddress, usdaAddress, weightX, weightY, wbtcPrice*ONE_16, 0);
        position = result.expectOk().expectTuple();
        position['dx'].expectUint(148648346);
        position['dy'].expectUint(wbtcPrice*ONE_16);        
...
//         result = FWPTest.reducePosition(deployer, wstxAddress, usdaAddress, weightX, weightY, wrongPooltokenAddress, ONE_16);
//         result.expectErr().expectUint(2026);

//         // Reduce all liquidlity
//         result = FWPTest.reducePosition(deployer, wstxAddress, usdaAddress, weightX, weightY, fwpwstxusdaAddress, ONE_16);
//         position = result.expectOk().expectTuple();
//         position['dx'].expectUint(62500000000000);
...
//         result.expectOk().expectUint(0.1*ONE_16)
...
//         result = FWPTest.swapXForY(deployer, wbtcAddress, usdaAddress, weightX, weightY, ONE_16, 0);
//         position = result.expectOk().expectTuple();
//         position['dx'].expectUint(ONE_16);    // 10% fee charged on wstx-usda leg, so you don't see
//         position['dy'].expectUint(4545445000000);    // but notice dy is 10% less.
        
//         // fee : 0.1* ONE_16
//         // dx-net-fees : 0.9 * ONE_16
//         // fee-rebate : 0.05 * ONE_16

//         ROresult = FWPTest.getPoolDetails(wstxAddress, usdaAddress, weightX, weightY);
//         position = ROresult.result.expectOk().expectTuple();
//         position['balance-x'].expectUint(55277772500000); // ~50000000000000 + 0.95 * ONE_16 * wbtcPrice
//         position['balance-y'].expectUint(50000000000000 - 4545445000000); 

//         // Swapping 
//         result = FWPTest.swapYForX(deployer, wbtcAddress, usdaAddress, weightX, weightY, ONE_16*wbtcPrice, 0);
//         position = result.expectOk().expectTuple();
//         position['dx'].expectUint(132252659);    // Corresponding dx value
//         position['dy'].expectUint(ONE_16*wbtcPrice);    // 10% fee charged on wstx-usda leg, so you don't see
        
//         // fee : 0.1 * ONE_16 * wbtcPrice
//         // dx-net-fees : 0.9 * ONE_16 * wbtcPrice
//         // fee-rebate : 0.05 * ONE_16 * wbtcPrice

//         ROresult = FWPTest.getPoolDetails(wstxAddress, usdaAddress, weightX, weightY);
//         position = ROresult.result.expectOk().expectTuple();
//         position['balance-x'].expectUint(49203975232689); 
//         position['balance-y'].expectUint(50000000000000 - 4545445000000 + 0.95 * ONE_16*wbtcPrice); // 620532212500000 + 0.95 * ONE_16 * wbtcPrice (4750000000000)
...
//         result = FWPTest.reducePosition(deployer, wstxAddress, usdaAddress, weightX, weightY, fwpwstxusdaAddress, ONE_16+1);
...
//         // Swapping 
//         result = FWPTest.swapXForY(deployer, wstxAddress, usdaAddress, weightX, weightY, wbtcPrice*ONE_16, 0);
//         position = result.expectOk().expectTuple();
//         position['dx'].expectUint(wbtcPrice*ONE_16);
//         position['dy'].expectUint(4629624375000);   

//         // Attempts to trade more than limit
//         result = FWPTest.swapXForY(deployer, wstxAddress, usdaAddress, weightX, weightY, ONE_16*ONE_16, 0);
...
//         // let's do some arb
//         call = await FWPTest.getYgivenPrice(wstxAddress, usdaAddress, weightX, weightY, Math.round(ONE_16*1.1));
...
//         call = await FWPTest.getYgivenPrice(wstxAddress, usdaAddress, weightX, weightY, Math.round(ONE_16 * 1.1 * 0.95));
//         call.result.expectErr().expectUint(2002);
//         // we need to call get-x-given-price
//         call = await FWPTest.getXgivenPrice(wstxAddress, usdaAddress, weightX, weightY, Math.round(ONE_16 * 1.1 * 0.95));
...
//         call = await FWPTest.getYgivenX(wstxAddress, usdaAddress, weightX, weightY, 50000*ONE_16);
...
//         call = await FWPTest.getXgivenY(wstxAddress, usdaAddress, weightX, weightY, 50000*ONE_16);
