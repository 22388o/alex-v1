...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        alexToken.mintFixed(deployer, wallet_2.address, 200 * ONE_16);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        //mint alex tokens
        alexToken.mintFixed(deployer, wallet_1.address, 200 * ONE_16);
        ...
        const futuresPoolBlock = chain.mineBlock([
            ...
            futuresPool.addToPosition(wallet_1, alexTokenAddress, 1, stakedAlexAddress, 200 * ONE_16),
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        //mint alex tokens
        alexToken.mintFixed(deployer, deployer.address, 200 * ONE_16);
        alexToken.mintFixed(deployer, wallet_1.address, 200 * ONE_16);
        ...
        const setupBlock = chain.mineBlock([
            futuresPool.setActivationThreshold(deployer, 1),
            futuresPool.addToken(deployer, alexTokenAddress),
            futuresPool.setCoinbaseAmount(deployer, alexTokenAddress, ONE_16, ONE_16, ONE_16, ONE_16, ONE_16),
            futuresPool.registerUser(deployer, alexTokenAddress),
            futuresPool.registerUser(wallet_1, alexTokenAddress),
        ]);
        chain.mineEmptyBlockUntil(
        ...
        const reducePosBlock = chain.mineBlock([
            futuresPool.reducePosition(wallet_1, alexTokenAddress, 1, stakedAlexAddress, ONE_16)
        ]);

        let tuple: any = reducePosBlock.receipts[0].result.expectOk().expectTuple();
        tuple['staked-token'].expectUint(200e16);
        tuple['reward-token'].expectUint(32 * ONE_16);
    }
})