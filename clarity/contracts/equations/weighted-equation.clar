...
(define-read-only (get-invariant (balance-x uint) (balance-y uint) (weight-x uint) (weight-y uint))
    (begin
        (asserts! (is-eq (+ weight-x weight-y) ONE_16) ERR-WEIGHT-SUM)
    ...
    (begin
        ...
        (let 
            (
                ...
                (complement (if (<= ONE_16 power) u0 (- ONE_16 power)))
    ...
    (begin
        ...
        (let 
            (
                ...
                (ratio (if (<= power ONE_16) u0 (- power ONE_16)))
    ...
    (begin
        ...
        (let
            ...
            (let 
                ...
                (ok (mul-up balance-x (if (<= power ONE_16) u0 (- power ONE_16))))
            )
        )
    )   
)
    ...
    (begin
        ...
        (let
            ...
            (let 
                ...
                (ok (mul-up balance-y (if (<= ONE_16 power) u0 (- ONE_16 power))))
            )
        )
    )   
)
